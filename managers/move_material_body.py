...
def move_material_body(time, h, moved_body):
    ...
    for i in range(len(moved_body.material_points)):
        t = h  # Начинаем с t = h, чтобы избежать log(0)
        ...
        for n in range(int(time / h) + 1):
            ...
            f_3x = managers.v1_x.v1_x(t + butcher_table.c[2] * h, x_k + h * butcher_table.a[2][1] * f_2x)
            
            f_1y = managers.v2_y.v2_y(t, y_k)
            f_2y = managers.v2_y.v2_y(t + butcher_table.c[1] * h, y_k + h * butcher_table.a[1][0] * f_1y)
            f_3y = managers.v2_y.v2_y(t + butcher_table.c[2] * h, y_k + h * butcher_table.a[2][1] * f_2y)
            
            x_t.append(x_k + h * (butcher_table.b[0] * f_1x + butcher_table.b[1] * f_2x + butcher_table.b[2] * f_3x))
            y_t.append(y_k + h * (butcher_table.b[0] * f_1y + butcher_table.b[1] * f_2y + butcher_table.b[2] * f_3y))
